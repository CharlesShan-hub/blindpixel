...
class FastBlindLoss(_Loss):
    ...
    def forward(self, input: Tensor, target: Tensor, mask: Tensor) -> Tensor:
        ...
        loss_ssim = ssim_loss(input, target, 7)
        loss_psnr = psnr_loss(input, target, 1.0)
        loss_mse = F.mse_loss(input, target, reduction=self.reduction)
        return loss_ssim + loss_psnr/1000 + loss_mse
...
class FastBlindNetTrainer(BaseTrainer):
    ...
    def holdout_train(self, epoch):
        ...
        for noisy, clean, mask in pbar:
            ...
            loss = self.criterion(outputs, clean, mask)
            loss.backward()
            self.optimizer.step()
